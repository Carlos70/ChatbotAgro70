...
export default function App() {
  ...
  const MANUALS_LIBRARY = [
    { id: 'pepino', name: 'Guía Centa Pepino', file: encodeURI('/Guia Centa_pepino.pdf') },
    { id: 'papa', name: 'Guía Centa Papa', file: encodeURI('/Guia Centa_papa.pdf') },
    { id: 'arroz', name: 'Guía Centa Arroz', file: encodeURI('/Guia Centa_Arroz.pdf') },
  ];
  ...
  const extractTextFromPDF = async (data: ArrayBuffer) => {
    try {
      ...
      const cleanedText = fullText.replace(/--- PÁGINA \d+ ---/g, '').trim();
      
      if (cleanedText.length < 50) { // Si hay menos de 50 caracteres en todo el PDF
  ...
  const loadManualFromLibrary = async (manual: typeof MANUALS_LIBRARY[0]) => {
    setIsProcessingFile(true);
    setMessages(prev => [...prev, { text: `Cargando manual oficial: **${manual.name}**...`, type: 'bot-msg', isLoading: true }]);

    try {
      const response = await fetch(manual.file);
      if (!response.ok) throw new Error("No se pudo encontrar el archivo en el servidor.");
      ...
      setMessages(prev => {
        ...
        return [...filtered, {
          text: `He cargado el **${manual.name}**. Ya puedes hacerme preguntas técnicas sobre este cultivo.`,
          type: 'bot-msg'
        }];
      });
    } catch (error) {
      console.error('Error al cargar manual de la biblioteca:', error);
      const errorMsg = error instanceof Error ? error.message : "Error desconocido";
      setMessages(prev => {
        const filtered = prev.filter(m => !m.isLoading);
        return [...filtered, {
          text: `No pude cargar el manual predefinido (${errorMsg}). Asegúrate de que los archivos estén en la carpeta **/public/** y que los nombres coincidan exactamente.`,
          type: 'bot-msg'
        }];
      });
    } finally {
